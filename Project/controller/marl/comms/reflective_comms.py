...
class ReflectiveComms(Comms, nn.Module):
    ...
    def __init__(self, config: CommConfig, actor_config: ActorHyperparameters, log: Callable[[str, float], None], device: torch.device, **kwargs):
        ...
        self.aim_codebooks = nn.ParameterList([
            nn.Parameter(
                F.normalize(torch.randn(config.vocab_size, config.communication_size, device=device), dim=-1),
                requires_grad=False
            )
            for _ in range(1)
        ])
