...
from controller.marl.core.logger import Logging, MetricTracker
...
def fit_rq_stream(model, residuals, quantiser, latent_dim, device: torch.device):
    ...
    for rq_level in range(quantiser.rq_levels):
        ...
        if isinstance(model.encoder, SQ_VAE) or isinstance(model.encoder, HQ_VAE):
            train_residuals = curr_residuals / (np.linalg.norm(curr_residuals, axis=-1, keepdims=True) + 1e-5)
        else:
            train_residuals = curr_residuals
        kmeans.fit(train_residuals)
        ...
        cluster_assignments = kmeans.predict(train_residuals)        
...
def train_language(system, config: Config, device: torch.device, use_optimal: bool = False):
    ...
    prior_set, train_set, val_set, test_set = torch.utils.data.random_split(dataset, [0.1, 0.7, 0.15, 0.05])
