...
from ..core.config import CommunicationType, Config, GenerativeLangType
...
def train(system, config: Config, device: torch.device):
    ...
    for current_training_timestep in range(system["start_step"], config.training.training_timesteps):
        ...
        fade_out_steps = 2000.0
        actor.comm_dropout_prob = max(0.0, 0.2 * (1.0 - (current_training_timestep / fade_out_steps)))
        ...
        for batch_run in range(rollout_phases):
            ...
            buf_global_obs = torch.zeros((T, W, sim.get_global_obs_size(0)), dtype=torch.float32, device=device)#  + N * NC * C
            ...
            for t in range(T):
                ...
                next_obs, next_global_obs, targets, rewards = sim.parallel_step(action_choice)
        ...
        for _ in range(config.mappo.ppo_epochs):
            ...
            for batch in buffer.get_minibatches(batch_size=config.training.batch_size):
                ...
                comm_ent_coef = 0.001
                entropy_loss = -action_entropy.mean() + (comm_ent_coef / config.mappo.ent_coef) * -comm_entropy.mean()
