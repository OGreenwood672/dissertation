...
df = pd.DataFrame(normalised_matrix, index=code_labels, columns=target_labels)

# %%
plt.figure(figsize=(10, 8))
sns.heatmap(df, cmap="Blues", fmt=".2f", cbar_kws={'label': 'Probability: P(Target | Code)'})
...
sns.heatmap(plot_matrix, cmap="YlGnBu", vmin=0.0, vmax=1.0, fmt=".2f", annot_kws={"size": 6}, cbar_kws={'label': 'P(Code | Target)'})
...
global_code_usage = temporal_matrix.sum(axis=(0, 2))
top_code_indices = np.sort(np.argsort(global_code_usage)[-5:])
