...
Ultimately, the evaluation suggests that the AI Mother Tongue architecture enables semantically meaningful discrete communication, outperforming both on-policy discrete and no-communication baselines by overcoming the Communication Vacuum Equilibrium in simple tasks.
...
For basic tasks, WhisperWise significantly outperforms the AI Mother Tongue architecture (reward mean per step improvement of $\sim 40\%$; see Section \ref{sec:4.8}) and achieves a steeper, more efficient semantic compression.
WhisperWise performed better in the tested scaling task, where AI Mother Tongue had limited success.
...
Finally, a third direction is to further research the WhisperWise protocol.
WhisperWise (as addressed in Section \ref{sec:4.9}) requires evaluation over a significantly larger range of random seeds ($\geq 30$).
WhisperWise's next step is to be integrated into a real-world application.
